...
class OnlinePreprocessor(torch.nn.Module):
    ...
    def forward(self, wavs=None, feat_list=None, wavs_len=None):
        ...
        print('baseline_preprocessor_wav',wav)
        #wavs=wavs.to('cuda')
        shape = wavs.size()
        #self._window.cuda()
        print('upstream/baseline/preprocessor.py:forward_beforecomplex',wavs)
        #self._window.to(wavs.device)
        #self._window.to('cuda')
        self._window=self._window.to(wavs.device)
        print('self._window.device',self._window.device)
        ...
        print('linear.device',linear.device)
        print('phase.device',phase.device)
