...
class Featurizer(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        print('.to(upstream_device)',(upstream_device))
        print('Featurizer.paired_wavs_beforepreprocess',paired_wavs)
        print('interface_pairedwavs',paired_wavs)
        #paired_wavs=self.preprocess(paired_wavs).to(upstream_device)
        
        #paired_wavs=paired_wavs.to(upstream_device)
        #print('paired_wavs.device',paired_wavs.device)
        with torch.no_grad():
            
            paired_features = upstream(paired_wavs)#.to(upstream_device)
            #print('paired_features.device',paired_features.device)
            print('paired_features',paired_features)
    ...
    def preprocess(self,x):
        self_config={'target_level': -25, 'win_ms': 25, 'hop_ms': 10, 'n_freq': 201, 'n_mels': 80, 'n_mfcc': 13,                               'input': {'feat_type': 'mel', 'channel': 0, 'log': True, 'delta': 0, 'cmvn': True}, 
                      'target': {'feat_type': 'mel', 'channel': 0, 'log': True, 'delta': 0, 'cmvn': True}}
        x=[self.normalize_wav_decibel(x_i).to('cuda') for x_i in x]
        x_lens=[len(x_) for x_ in x]
        x=pad_sequence(x,batch_first = True)
        x=x.unsqueeze(1).to('cuda')
        print('preprocess.x',x)
        extracter,_,_ = get_preprocessor(self_config)
        x=extracter(x,wavs_len=x_lens)[0].to('cuda')
        return x
    ...
    def _weighted_sum(self, feature):
        ...
        norm_weights=norm_weights.to(stacked_feature.device)
