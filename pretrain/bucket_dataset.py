...
class FeatDataset(Dataset):
    ...
    def __init__(self, extracter, task_config, bucket_size, file_path, sets, 
        ...
        print('FeatDataset_max_timestep',max_timestep)
        ...
        # Read file
        self.root = file_path
        #print('FeatDataset_self.root',self.root)
        #print('FeatDataset_sets',sets)

        tables = [pd.read_csv(os.path.join(file_path, s + '.csv')) for s in sets]
        #print('FeatDataset_tables',tables[0])
        self.table = pd.concat(tables, ignore_index=True).sort_values(by=['length'], ascending=False)
        #print('FeatDataset_self.table\n',self.table)
        print('[Dataset] - Training data from these sets:', str(sets))

        # Drop seqs that are too long
        print('FeatDataset_max_timestep2',max_timestep)
        print('FeatDataset_self.table.length1\n',self.table.length)
        if max_timestep > 0:
            self.table = self.table[self.table.length < max_timestep]
        # Drop seqs that are too short
        if max_timestep < 0:
            self.table = self.table[self.table.length > (-1 * max_timestep)]
        print('FeatDataset_self.table.length2',self.table.length)
        print('FeatDataset_self.table2\n',self.table)

        X = self.table['file_path'].tolist()
        print('X',X)
        X_lens = self.table['length'].tolist()
        #print('FeatDataset_X',X)
