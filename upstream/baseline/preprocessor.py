...
class OnlinePreprocessor(torch.nn.Module):
    ...
    def forward(self, wavs=None, feat_list=None, wavs_len=None):
        ...
        print('upstream/baseline/preprocessor.py:forward')
        print(os.getcwd())
        ...
        print('upstream/baseline/preprocessor.py:forward.wav',wav)
        shape = wavs.size()
        print('upstream/baseline/preprocessor.py:forward_beforecomplex',wavs)
        complx = self._stft(wavs.reshape(-1, shape[-1]), window=self._window)
        print('complx.shape',complx.size())
        ...
        def mean_spectrogram(tensor):
            ...
            #print('device',device)
            #len_k=len(data)
            for _,data in enumerate(tensor):
                data_shape=data.shape
                mean_data= torch.zeros(data_shape[0],data_shape[1],data_shape[2]).to(device)
                for p, data_p_188_80 in enumerate(data):
                    len_i=data_shape[1]
                    len_j=data_shape[2]
                    Mean=torch.mean(input=data_p_188_80, dim=0)
                    for i in range(len_i):
                        for j in range(len_j):
                            mean_data[p][i][j]=Mean[j]#.to(device)
