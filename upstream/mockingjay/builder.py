...
class TransformerBuilder(nn.Module):
    ...
    def __init__(self, options, inp_dim=-1, config=None, on_the_fly_config=None, verbose=False):
        ...
        print('self.config[audio]',self.config['audio'])
...
class PretrainedTransformer(TransformerBuilder):
    ...
    def forward(self, x):
        #print('builder.py:x',x.shape)
        if self.extracter is not None:
            if 'kaldi' in self.config['audio']:
                x = [self.extracter(x_i) for x_i in x]
                x = pad_sequence(x, batch_first=True)
            else:
                print('builder.py:self.extracter is None')
                print('PretrainedTransformer.x_before_Extracter',x)
                x = [self._normalize_wav_decibel(x_i) for x_i in x]
                print('PretrainedTransformer.x_After__normalize_wav_decibel',x)
                x_lens = [len(x_) for x_ in x]
                x = pad_sequence(x, batch_first=True)
                x = x.unsqueeze(1) # (batch_size, audio_len) -> (batch_size, 1, audio_len)
                print('PretrainedTransformer.x_After_unsqueeze',x)
                #print('PretrainedTransformer.x_before_Extracter',x)
                x = self.extracter(x, wavs_len=x_lens)[0]
                print('PretrainedTransformer.x_after_Extracter',x)
                print('PretrainedTransformer.x',x.size())
                torch.save(x, '/home/negiryosuke/refine/s3prl/visualaization/builder_tensor_296.pt')
        if self.no_grad:
            with torch.no_grad():
                x = self._forward(x)
        else:
            print('upstream/mockingjay/builder.py',x)
...
class PretrainedTransformerWithHead(PretrainedTransformer):
    ...
    def forward(self, x):
        print('builder.py:x',x.shape)
        if self.extracter is not None:
            print('builder.py:self.extracter is not None')
            if 'kaldi' in self.config['audio']:
                print('builder.py:self.extracter is not None')
                print('builder.py:kaldi')
                x = [self.extracter(x_i) for x_i in x]
                x = pad_sequence(x, batch_first=True)
            else:
                x = [self._normalize_wav_decibel(x_i) for x_i in x]
